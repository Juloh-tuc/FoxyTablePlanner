          ...
          {tasks.length ? tasks.map((t) => (
            <div key={t.id} className="pm-item">
              <div className="pm-left">
                <div className="pm-title" title={t.titre}>{t.titre}</div>
                <div className="pm-meta">
                  ...
                  {t.remarques && <span className="pm-remarks one-line" title={t.remarques}>{t.remarques}</span>}
                </div>
              </div>
              <div className="pm-actions">
                <button className="ft-btn" onClick={() => onEdit(t.id)} disabled={meeting}>Éditer</button>
                <button className="ft-btn ghost" onClick={() => onDetail(t.id)}>Détails</button>
                <button className="ft-btn" onClick={() => onArchive(t.id)} disabled={meeting}>Archiver</button>
                <button className="ft-btn danger" onClick={() => onDelete(t.id)} disabled={meeting}>Supprimer</button>
              </div>
            </div>
...
export default function PlannerMonth() {
  ...
  const [meeting, setMeeting] = useState<boolean>(() => {
    try { return localStorage.getItem("planner.month.meeting") === "1"; }
    catch { return false; }
  });
  const toggleMeeting = () => {
    setMeeting((v) => {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((t) => {
      ...
      if (!text) return true;
      const hay = [
        t.titre, t.admin, t.priorite ?? "", t.remarques ?? "", t.bloque ?? "", t.bloquePar ?? "",
        ...(t.etiquettes ?? []),
      ].join(" ").toLowerCase();
  ...
  const dayPeople = useMemo(() => {
    ...
    filtered.forEach((t) => {
      if (t.debut && t.echeance) {
        ...
        while (cur <= end) { push(fmt(cur), t); cur.setDate(cur.getDate() + 1); }
  ...
  const [personDay, setPersonDay] = useState<{ open: boolean; person: string | null; dateISO: string | null; }>(
  ...
  const goToday = () => { setCursor(new Date()); setPingToday(true); };
  ...
  return (
    <section className={`month-wrap ${meeting ? "meeting-on" : ""}`}>
      ...
      {/* Barre d’actions (nav + filtres + toggle) */}
      <header className="month-toolbar">
        <div className="nav">
          <button className="btn" onClick={() => setCursor(d => new Date(d.getFullYear(), d.getMonth() - 1, 1))}>◀</button>
          <button className="btn today" onClick={goToday} title="Revenir au mois actuel">Aujourd’hui</button>
          <button className="btn" onClick={() => setCursor(d => new Date(d.getFullYear(), d.getMonth() + 1, 1))}>▶</button>
        </div>

        <div className="right">
          <div className="filters">
            <input className="flt flt-input" placeholder="Rechercher…" value={q} onChange={(e) => setQ(e.target.value)} />
          ...
          <button className={`meeting-toggle ${meeting ? "is-on" : ""}`} onClick={toggleMeeting} title="Basculer Mode réunion">
      ...
      <div className="month-grid month-grid--head">
        {["Lun","Mar","Mer","Jeu","Ven","Sam","Dim"].map(d => <div key={d} className="day-head">{d}</div>)}
      </div>

      {/* Grille */}
      <div className="month-grid">
        {days.map(({ date, inMonth, isToday }) => {
          const bucket = dayPeople.get(date) ?? new Map();
          const entries = Array.from(bucket.entries()); // [person, {count,tasks}]
                    ...
                    return (
                      ...
                      >
                        <span className="avatar">
                          {img ? <img src={encodeURI(img)} alt="" /> : <span className="fallback">{initials(person) || "?"}</span>}
                        </span>
      ...
      <PersonDayModal
        ...
        onAddTask={(p, iso) => { setPersonDay({ open: false, person: null, dateISO: null }); quickNewForDay(iso, p); }}
        onEdit={(id) => { if (meeting) return; const t = rows.find(x => x.id === id); setPersonDay({ open: false, person: null, dateISO: null }); if (t) setEditTask(t); }}
        onDetail={(id) => { const t = rows.find(x => x.id === id); setPersonDay({ open: false, person: null, dateISO: null }); if (t) setOpenTask(t); }}
        onArchive={(id) => { if (meeting) return; setRows(prev => prev.map(x => x.id === id ? { ...x, archived: true, archivedAt: new Date().toISOString() } : x)); }}
        onDelete={(id) => { if (meeting) return; const t = rows.find(x => x.id === id); setPersonDay({ open: false, person: null, dateISO: null }); if (t) askArchiveDeleteFor(t); }}
      />
      ...
      {openTask && (
        <TaskDetailModal
          ...
          onEdit={(taskId: string) => { if (meeting) return; const t = rows.find(x => x.id === taskId); if (t) setEditTask(t); }}
          onDelete={(taskId: string) => { const t = rows.find(x => x.id === taskId); if (t) askArchiveDeleteFor(t); }}
        />
      )}
      {editTask && !meeting && <EditTaskModal task={editTask} onClose={() => setEditTask(null)} onSave={saveTask} />}
      ...
      {/* === Badge fixe Mode réunion === */}
      {meeting && (
        <div className="meeting-badge" aria-live="polite">
          <span className="dot" /> Mode réunion ON
        </div>
      )}
    </section>
  );
